...
# 文件权限位：BSD/macOS 用 stat -f '%Lp'，GNU/Linux 用 stat -c '%a'。
# 兼容两者，使脚本可在成本低得多的 ubuntu runner 上执行
# （macos runner 私有仓库计费倍率为 10x）。
# 注意不能用 `stat -f ... || stat -c ...` 回退：GNU stat 的 -f 是"查询文件
# 系统信息"而非格式化，在 Linux 上会输出一段文件系统摘要，导致误判。
file_mode() {
