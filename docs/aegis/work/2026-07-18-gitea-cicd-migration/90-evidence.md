...
- Artifact key: task11-feature-ci-and-protected-pr
- Type: live Gitea CI, rootless DinD compatibility, status-context, protection, and internal-PR proof
- Source: commits `93304eafa`, `fa4178c90`, `ab5fff2d6`, `b8fe2f72a`, and `af84a46d4`; Gitea runs 30/31 and PR 1 on 2026-07-19
- Summary: Added the digest-locked private Go+Node Actions image, bounded Go package/compiler concurrency, raised DinD to a still-bounded 4 GiB, routed Testcontainers published-port and Ryuk socket access through the outer `docker` service/inner Unix socket contract, and obtained green push CI plus security aggregates. Activated `main` and `release/v*` protection with the live-proven contexts `ci / required (push)` and `security / required (push)`, then opened the internal feature-to-main PR without merging it.
- Verifier: Runs 30/31 contained nine successful jobs; deploy had zero runs; the DinD OOM counter remained at its pre-run value 2; Runner remained online, UID/GID 1000 and non-privileged, while DinD remained the sole privileged container with a 4 GiB/3 CPU boundary and mode-1660 UID/GID-1000 Unix socket. The PR run's first internal-repository guard and pinned checkout step both succeeded.
- Credential/cleanup evidence: Each temporary `luoee` PAT had only `write:repository`, lived in root-owned mode-0600 `/run` files, and was revoked with zero matching rows/files afterward. Ten non-running `org.testcontainers=true` containers from earlier failed Ryuk checks were removed; no labeled residual remained before PR creation.
- Residual boundary: PR pull-request runs 32/33/34 are still executing. PR 1 is open and unmerged. Registry candidate, backup/restore drill, Gateway cutover, GitHub Actions disablement, and every Task 13 production action remain pending.
