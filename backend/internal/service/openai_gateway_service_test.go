...
func TestOpenAIStreamingKeepaliveBeforeClientOutputPreservesFailover(t *testing.T) {
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
			StreamKeepaliveInterval:   1,
	...
	case err := <-done:
		var failoverErr *UpstreamFailoverError
		require.ErrorAs(t, err, &failoverErr)
		require.True(t, c.Writer.Written(), "stable keepalive may commit the HTTP response")
		require.Equal(t, -1, OpenAICompactKeepaliveAdjustedWrittenSize(c), "keepalive bytes must not count as semantic output")
		require.Contains(t, rec.Body.String(), ":\n\n")
