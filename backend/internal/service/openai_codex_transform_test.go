...
func TestApplyCodexOAuthTransform_ObjectSystemInputBecomesDeveloperAndInstructions(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.4",
		"input": map[string]any{
			"type":    "message",
	...
	result := applyCodexOAuthTransform(reqBody, false, false)
	require.True(t, result.Modified)
	require.Equal(t, "system instructions", reqBody["instructions"])
	...
	require.Equal(t, "system instructions", msg["content"])
}

func TestApplyCodexOAuthTransform_ObjectItemReferencePreserved(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.4",
		"input": map[string]any{
			"type": "item_reference",
	...
	require.True(t, ok)
	require.Len(t, input, 1)
	ref, ok := input[0].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "item_reference", ref["type"])
	require.Equal(t, "rs_123", ref["id"])
}
