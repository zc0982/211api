		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if !clientDisconnected && !failureDelivered && !suppressCurrentEvent {
				...
				if (firstTokenMs == nil && startsVisibleOutput) ||
					(startsClientOutput && !startsVisibleOutput && !openAIStreamEventTypeIsTerminal(eventType)) {
					// 首个可见输出与非可见的结构性进度都要尽快出站。前者保证 TTFT，
					// 后者提交本次尝试并让后续 keepalive 按下游空闲时间工作。
					shouldFlush = true
				}
