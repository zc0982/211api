...
func TestEasyPayCreatePaymentMapsMAPIResponseLaunchFields(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "uses urlscheme when payurl is absent",
			response:    `{"code":1,"trade_no":"trade-urlscheme","urlscheme":"weixin://dl/business/?t=launch"}`,
			wantTradeNo: "trade-urlscheme",
			wantPayURL:  "weixin://dl/business/?t=launch",
		},
		{
			name:        "prefers payurl over urlscheme",
			response:    `{"code":1,"trade_no":"trade-payurl","payurl":"https://pay.example.com/launch","urlscheme":"weixin://dl/business/?t=fallback"}`,
			wantTradeNo: "trade-payurl",
			wantPayURL:  "https://pay.example.com/launch",
		},
		{
			name:        "prefers payurl2 for mobile requests",
			response:    `{"code":1,"trade_no":"trade-mobile","payurl":"https://pay.example.com/desktop","payurl2":"https://pay.example.com/mobile"}`,
			isMobile:    true,
			wantTradeNo: "trade-mobile",
			wantPayURL:  "https://pay.example.com/mobile",
		},
		{
			name:        "preserves qrcode",
			response:    `{"code":1,"trade_no":"trade-qrcode","qrcode":"weixin://wxpay/bizpayurl?pr=qr"}`,
			wantTradeNo: "trade-qrcode",
			wantQRCode:  "weixin://wxpay/bizpayurl?pr=qr",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := provider.CreatePayment(context.Background(), payment.CreatePaymentRequest{
				...
				Subject:     "EasyPay create payment",
